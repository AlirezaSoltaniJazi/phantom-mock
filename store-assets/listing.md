...
Mock REST API responses and override request/response headers from your
browser. Group rules, toggle them on the fly, no server needed.

## Long description

Phantom Mock is a developer-focused Chrome extension that lets you intercept
REST API traffic right inside the browser — without spinning up a separate
mock server.
...
when, and by which rule.

**Import / export.** Share rule sets with your team via a single JSON file,
with merge strategies for replacing, merging by id, or appending as new.

Phantom Mock works fully offline. No accounts, no telemetry, no servers —
all rules live in your browser's local storage.
